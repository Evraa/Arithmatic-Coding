...
public:
	...
	bitset<32> decimal_to_binary(double fraction,int k);
	...
	// i.e. one entry (tag) for every block of symbols.//Needs to be calculated :(
	//
	// The function should return the number of bits in the encoded message.
	...
	void printOutMessage(int k, vector<string>* encoded_msg);
